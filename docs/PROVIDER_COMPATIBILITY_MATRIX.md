...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| OpenRouter | `OPENROUTER_BASE_URL` or `CODEX_PROVIDER_BASE_URL` | `OPENROUTER_MODEL` or `CODEX_PROVIDER_MODEL` | OpenAI-compatible Chat Completions through local Responses adapter | `mixed`; `pure-api` for API-key-only hosts | Custom tools and adapter-emulated hosted tools through CodexProvider | Verified through adapter SSE path | Verified with `deepseek/deepseek-chat`; some models may ignore forced tools | Verified through adapter-emulated source | Verified with built-in no-key metasearch; API-backed search supports Brave/SerpApi/Serper/Tavily when credentials are present | Model-specific tool-call behavior varies | Passed: see [live smoke results](LIVE_SMOKE_RESULTS.md#2026-06-10t215515241z-codexproviderruntime-live-host-integration-smoke) |
| DeepSeek official | `DEEPSEEK_BASE_URL` or `CODEX_PROVIDER_BASE_URL` | `DEEPSEEK_MODEL` or `CODEX_PROVIDER_MODEL` | OpenAI-compatible Chat Completions through local Responses adapter | `mixed`; `pure-api` for API-key-only hosts | Verified custom tools and adapter-emulated hosted tools | Verified through adapter SSE path | Verified with `deepseek-chat` forced custom tool call | Verified through adapter-emulated source | Verified with built-in no-key metasearch and API-backed SerpApi | Official base URL may omit `/v1`; use preset unless provider docs require override | Passed: see [live smoke results](LIVE_SMOKE_RESULTS.md#2026-06-30t170648247z-adapter-emulated-web_search-live-smoke) |
| DashScope/Qwen | `DASHSCOPE_BASE_URL` or `QWEN_BASE_URL` | `DASHSCOPE_MODEL` or `QWEN_MODEL` | OpenAI-compatible Chat Completions through local Responses adapter | `mixed`; `pure-api` for API-key-only hosts | Verified custom tools and adapter-emulated hosted tools; Qwen preset records native search capability metadata | Verified through adapter SSE path | Verified with `qwen-plus` forced custom tool call | Verified through adapter-emulated source | Verified with built-in no-key metasearch; native `chat_enable_search` must remain provider-specific | Some deployments use `DASHSCOPE_*`, others use `QWEN_*` env names | Passed: see [live smoke results](LIVE_SMOKE_RESULTS.md#2026-06-29t232033021z-codexproviderruntime-live-host-integration-smoke) |
...
Future cycles should add a SiliconFlow live behavior record, remaining provider-preset live records, and an OpenAI direct Responses recipe after the product default and credentials are available.
