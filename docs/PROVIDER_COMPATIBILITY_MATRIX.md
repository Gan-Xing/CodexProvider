...
| SiliconFlow | `SILICONFLOW_BASE_URL` or `CODEX_PROVIDER_BASE_URL` | `SILICONFLOW_MODEL` or `CODEX_PROVIDER_MODEL` | OpenAI-compatible Chat Completions through local Responses adapter | `mixed`; future preset needed | Not yet verified | Not yet verified | Needs live forced-tool verification | Expected only through adapter-emulated source | Expected through adapter-emulated search | Provider-specific model/tool-call behavior not recorded | [!] Pending credentials |
| MiniMax | `MINIMAX_BASE_URL` or `CODEX_PROVIDER_BASE_URL` | `MINIMAX_MODEL` or `CODEX_PROVIDER_MODEL` | OpenAI-compatible Chat Completions through local Responses adapter | `mixed`; future profile helper can reuse capability preset | Expected custom tools and adapter-emulated hosted tools | Expected through adapter SSE path | Needs live forced-tool verification | Expected through adapter-emulated source | Expected through adapter-emulated search | Text-only multimodal capability metadata is recorded in the capability preset | [!] Pending credentials |
| Moonshot/Kimi | `KIMI_BASE_URL` or `CODEX_PROVIDER_BASE_URL` | `KIMI_MODEL` or `CODEX_PROVIDER_MODEL` | OpenAI-compatible Chat Completions through local Responses adapter | `mixed`; future profile helper can reuse capability preset | Expected custom tools and adapter-emulated hosted tools | Expected through adapter SSE path | Needs live forced-tool verification | Expected through adapter-emulated source | Expected through adapter-emulated search | Kimi coding endpoint defaults differ from generic OpenAI base URLs | [!] Pending credentials |
...
Cycle 1 adds root profile helpers for:
...
Future cycles should add profile helpers or documented recipes for SiliconFlow, MiniMax, Moonshot/Kimi, and OpenAI direct Responses after live behavior is recorded.
