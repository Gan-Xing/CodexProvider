...
| 18 | P2 | Deep search is currently heuristic | Partially done | `docs/DEEP_WEB_SEARCH_ROADMAP.md` documents heuristic opt-in status, planner interface, graph execution, reference merge, and synthesis contract. Cycle 10 adds heuristic planner diagnostics plus decomposition, graph-rejection, and reference-merge fixture coverage. Cycle 11 adds failed-subquery, unresponsive-engine, and planner-budget diagnostics plus partial-failure and executor-budget coverage. Cycle 12 adds recipe/example coverage that keeps `custom:deep_web_search` separate from default `web_search`. Cycle 13 adds no-supporting-evidence response and executor metadata. Cycle 14 adds optional minimum-source diagnostics and limited-evidence synthesis guidance. Cycle 15 adds optional citation-budget controls and capped citation guidance. Cycle 16 adds optional answer-shape guidance for brief answers, evidence tables, and research memos. Cycle 17 adds graph budget and duration diagnostics. Cycle 18 adds opt-in per-node timeout budgets, retries, and attempt/timeout metadata. It remains opt-in and not a recommended default research product. |
...
Recursive quality Cycle 4 validation run on 2026-06-11:
...
pnpm check-boundary                            # passed
pnpm check-package-surface                     # passed
pnpm pack:dry-run                              # passed: 593 files, 348.7 kB package size, 1.6 MB unpacked
```
...
Recursive quality Cycle 6 validation run on 2026-06-11:
...
pnpm check-boundary                            # passed
pnpm check-package-surface                     # passed
pnpm pack:dry-run                              # passed: 593 files, 350.4 kB package size, 1.6 MB unpacked
```
...
Recursive quality Cycle 8 validation run on 2026-06-11:
...
pnpm check-boundary                            # passed
pnpm check-package-surface                     # passed
pnpm pack:dry-run                              # passed: 593 files, 352.2 kB package size, 1.6 MB unpacked
```

Additional focused and optional validation:

```bash
pnpm exec tsx --test test/web_search_executor.test.ts test/web_search_responses_output.test.ts  # passed: 25 tests
credential presence check                                                                       # upstream and API-backed search credentials missing; live smokes skipped
git diff --check                                                                                # passed
```

Recursive quality Cycle 9 validation run on 2026-06-11:

```bash
node scripts/recursive-quality-cycle.mjs scan  # passed: 0 findings
pnpm test                                      # passed: 284 passing, 1 credential-gated integration skipped
pnpm typecheck                                 # passed
pnpm build                                     # passed
pnpm consumer:harness                          # passed
pnpm check-boundary                            # passed
...
Recursive quality Cycle 10 validation run on 2026-06-11:

```bash
node scripts/recursive-quality-cycle.mjs scan  # passed: 0 findings
pnpm test                                      # passed: 287 passing, 1 credential-gated integration skipped
pnpm typecheck                                 # passed
pnpm build                                     # passed
pnpm consumer:harness                          # passed
pnpm check-boundary                            # passed
pnpm check-package-surface                     # passed
pnpm pack:dry-run                              # passed: 593 files, 354.0 kB package size, 1.6 MB unpacked
```

Additional focused and optional validation:

```bash
pnpm exec tsx --test test/web_search_deep_search.test.ts test/public_surface.test.ts  # passed: 17 tests
pnpm exec tsx --test test/web_search_deep_search.test.ts                              # passed: 6 tests
credential presence check                                                             # upstream and API-backed search credentials missing; live smokes skipped
git diff --check                                                                      # passed
```

Recursive quality Cycle 11 validation run on 2026-06-11:

```bash
node scripts/recursive-quality-cycle.mjs scan  # passed: 0 findings
pnpm test                                      # passed: 289 passing, 1 credential-gated integration skipped
pnpm typecheck                                 # passed
pnpm build                                     # passed
pnpm consumer:harness                          # passed
pnpm check-boundary                            # passed
pnpm check-package-surface                     # passed
pnpm pack:dry-run                              # passed: 593 files, 354.6 kB package size, 1.6 MB unpacked
```

Additional focused and optional validation:

```bash
pnpm exec tsx --test test/web_search_deep_search.test.ts  # passed: 8 tests
credential presence check                                 # upstream and API-backed search credentials missing; live smokes skipped
git diff --check                                          # passed
```

Recursive quality Cycle 12 validation run on 2026-06-11:

```bash
node scripts/recursive-quality-cycle.mjs scan  # passed: 0 findings
pnpm test                                      # passed: 290 passing, 1 credential-gated integration skipped
pnpm typecheck                                 # passed
pnpm build                                     # passed
pnpm consumer:harness                          # passed
pnpm check-boundary                            # passed
pnpm check-package-surface                     # passed
pnpm pack:dry-run                              # passed: 593 files, 355.2 kB package size, 1.6 MB unpacked
```

Additional focused and optional validation:

```bash
pnpm exec tsx --test test/public_surface.test.ts  # passed: 12 tests
credential presence check                         # upstream and API-backed search credentials missing; live smokes skipped
git diff --check                                  # passed
```

Recursive quality Cycle 13 validation run on 2026-06-11:

```bash
node scripts/recursive-quality-cycle.mjs scan  # passed: 0 findings
pnpm test                                      # passed: 291 passing, 1 credential-gated integration skipped
pnpm typecheck                                 # passed
pnpm build                                     # passed
pnpm consumer:harness                          # passed
pnpm check-boundary                            # passed
pnpm check-package-surface                     # passed
pnpm pack:dry-run                              # passed: 593 files, 355.9 kB package size, 1.6 MB unpacked
```

Additional focused and optional validation:

```bash
pnpm exec tsx --test test/web_search_deep_search.test.ts  # passed: 9 tests
credential presence check                                 # upstream and API-backed search credentials missing; live smokes skipped
git diff --check                                          # passed
```

Recursive quality Cycle 14 validation run on 2026-06-11:

```bash
node scripts/recursive-quality-cycle.mjs scan  # passed: 0 findings
pnpm test                                      # passed: 292 passing, 1 credential-gated integration skipped
pnpm typecheck                                 # passed
pnpm build                                     # passed
pnpm consumer:harness                          # passed
pnpm check-boundary                            # passed
pnpm check-package-surface                     # passed
pnpm pack:dry-run                              # passed: 593 files, 356.8 kB package size, 1.6 MB unpacked
```

Additional focused and optional validation:

```bash
pnpm exec tsx --test test/web_search_deep_search.test.ts  # passed: 10 tests
credential presence check                                 # upstream and API-backed search credentials missing; live smokes skipped
git diff --check                                          # passed
```

Recursive quality Cycle 15 validation run on 2026-06-11:

```bash
node scripts/recursive-quality-cycle.mjs scan  # passed: 0 findings
pnpm test                                      # passed: 294 passing, 1 credential-gated integration skipped
pnpm typecheck                                 # passed
pnpm build                                     # passed
pnpm consumer:harness                          # passed
pnpm check-boundary                            # passed
pnpm check-package-surface                     # passed
pnpm pack:dry-run                              # passed: 593 files, 357.7 kB package size, 1.6 MB unpacked
```

Additional focused and optional validation:

```bash
pnpm exec tsx --test test/web_search_deep_search.test.ts  # passed: 12 tests
credential presence check                                 # upstream and API-backed search credentials missing; live smokes skipped
git diff --check                                          # passed
```

Recursive quality Cycle 16 validation run on 2026-06-11:
...
Recursive quality Cycle 18 validation run on 2026-06-11:

```bash
node scripts/recursive-quality-cycle.mjs scan  # passed: 0 findings
pnpm test                                      # passed: 296 passing, 1 credential-gated integration skipped
pnpm typecheck                                 # passed
pnpm build                                     # passed
pnpm consumer:harness                          # passed
pnpm check-boundary                            # passed
pnpm check-package-surface                     # passed
pnpm pack:dry-run                              # passed: 593 files, 361.4 kB package size, 1.6 MB unpacked
```

Additional focused and optional validation:

```bash
pnpm exec tsx --test test/web_search_deep_search.test.ts  # passed: 14 tests
credential presence check                                 # upstream and API-backed search credentials missing; live smokes skipped
git diff --check                                          # passed
```
