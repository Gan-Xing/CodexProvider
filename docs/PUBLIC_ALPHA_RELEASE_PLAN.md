...
- Treat current OpenRouter and DashScope/Qwen live evidence as necessary but not sufficient for public alpha; release-owner review is still required.
...
- Third provider live evidence is not available in this environment. `DEEPSEEK_API_KEY` is missing, so DeepSeek official cannot be used as the preferred third-provider smoke; SiliconFlow, MiniMax, Moonshot/Kimi, and OpenAI credentials are also missing.

## Readiness Audit

| Area | Current status | Public-alpha action |
| --- | --- | --- |
| `README.md` | Explains package goal, non-affiliation with OpenAI, profile modes, hosted tool boundaries, current two-provider evidence, and key docs. | Keep provider evidence summary aligned with the matrix. |
...
- Pending credentials: DeepSeek official, SiliconFlow, MiniMax, Moonshot/Kimi, OpenAI direct Responses, and API-backed Brave/Serper/Tavily search.
...
- At least two OpenAI-compatible provider paths have current live smoke evidence for normal response, forced custom tool, `file_search`, non-streaming `web_search`, and streaming `web_search`. Current evidence satisfies this for OpenRouter and DashScope/Qwen.
