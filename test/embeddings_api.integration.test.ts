...
const shouldRun = process.env.CODEX_PROVIDER_RUN_EMBEDDINGS_INTEGRATION === '1'
  && Boolean(process.env.EMBEDDINGS_API_KEY || process.env.OPENROUTER_API_KEY);

test('embeddings API provider returns real vectors', {
  skip: shouldRun
    ? false
    : 'set CODEX_PROVIDER_RUN_EMBEDDINGS_INTEGRATION=1 and EMBEDDINGS_API_KEY to run',
}, async () => {
  ...
  const result = await provider.embed([
    'Codex provider file search integration test',
